...
// 6. SWAP Optimized Variable Matcher
console.log("Applying Optimized Variable Matcher...");
const optSrc = path.join(MOD_SRC, 'src/lib/utils/variable-matcher.opt.ts');
const targetUtil = path.join(MOD_DEST_DIR, 'src/lib/utils/variable-matcher.util.ts');

if (fs.existsSync(optSrc)) {
    console.log(`Swapping in optimized matcher from ${optSrc}`);
    fs.copyFileSync(optSrc, targetUtil);
} else {
    console.error("Optimized matcher file not found!");
}

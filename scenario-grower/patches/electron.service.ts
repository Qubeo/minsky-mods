...
export class ElectronService {
    ...
    async readFileText(filePath: string): Promise<string> {
        return await this.ipcRenderer.invoke('read-file-text', filePath);
    }

    async writeTempFile(content: string, filename: string): Promise<string> {
        return await this.ipcRenderer.invoke('write-temp-file', content, filename);
    }
